...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, device='cuda', click_method='random', num_clicks=1, prev_masks=None):
    ...
    for click_idx in range(num_clicks):
        with torch.no_grad():
            ...
            prob_list.append(medsam_seg_prob)

    return pred_list, click_points, click_labels, prob_list
...
def run():
    ...
    pbar = tqdm(test_dataloader, desc="")
    for batch_data in pbar:
        image3D, gt3D, meta_info = batch_data
        img_name = meta_info["image_path"][0]
        img_name_vis = img_name
        pbar.set_description(f"Processing: {img_name_vis}")
        ...
        if (args.skip_existing_pred and osp.exists(pred_path)):
            ...
            prob3D_full_dict  = {click_idx:torch.zeros_like(gt3D_full).numpy() for click_idx in range(args.num_clicks)}
            ...
            for (image3D, gt3D, pos3D) in sliding_window_list:
                # 3D inference
                seg_mask_list, points, labels, prob_list = finetune_model_predict3D(
                ...
                for idx, prob in enumerate(prob_list):
                    prob_roi = prob[..., pred_roi[0]:pred_roi[1], pred_roi[2]:pred_roi[3], pred_roi[4]:pred_roi[5]]
                    prob3D_full_dict[idx][..., ori_roi[0]:ori_roi[1], ori_roi[2]:ori_roi[3], ori_roi[4]:ori_roi[5]] = prob_roi
            ...
            for idx, prob3D_full in prob3D_full_dict.items():
                save_numpy_to_nifti(prob3D_full, osp.join(vis_root, osp.basename(img_name).replace(".nii.gz", f"_prob{idx}.nii.gz")), meta_info)
